...
export default function Worlds() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-3xl font-black">{t('pickWorldHeader')}</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400">{t('pickWorldSub')}</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        {WORLDS.map((w, i) => {
          const active = worldId === w.id;
          const name = locale === 'en' ? w.nameEn : w.name;
          const desc = locale === 'en' ? w.descriptionEn : w.description;
          return (
            <motion.button
              key={w.id}
              ...
              onClick={() => { sfx.tap(); haptic(); setWorld(w.id); }}
              aria-pressed={active}
            ...
            >
              ...
              <div className="relative">
                ...
                <div className="font-black text-lg leading-tight text-slate-900 dark:text-white">{name}</div>
                <div className="text-xs text-slate-700 dark:text-slate-200 opacity-80">{desc}</div>
